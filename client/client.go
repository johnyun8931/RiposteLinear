...
func initializeUploadArgs(args *db.UploadArgs, xIdx int, yIdx int,
  ...
  for i := 0; i < db.NUM_SERVERS; i++ {
    copy(args.Query[i].XCoords[:], randVecsX[:])
    copy(args.Query[i].YCoords[:], randVecsY[:])

    if (i & 1) == 0 {
      args.Query[i].XCoords[xIdx] = !args.Query[i].XCoords[xIdx]
    }

    if (i & 2) == 0 {
      old := args.Query[i].YCoords[yIdx]
      args.Query[i].YCoords[yIdx] = db.AddSlots(old, msg)
    }
  }
