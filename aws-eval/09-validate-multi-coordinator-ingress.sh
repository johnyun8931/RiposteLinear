...
MULTI_EPOCH_SECONDS="${MULTI_EPOCH_SECONDS:-20}"
...
deadline=$((SECONDS + MULTI_LEASE_TTL_SECONDS + 60))
while true; do
  if wait_for_status_state coordinator "$COORDINATOR_PUBLIC_IP" "$(coordinator_standby_addr)" active 1 >/dev/null 2>&1; then
    break
  fi
